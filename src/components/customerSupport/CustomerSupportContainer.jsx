...
const CustomerSupportContainer = () => {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_APP_API_ENDPOINT}/api/store/get-all-tickets`)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(
        `${import.meta.env.VITE_APP_API_ENDPOINT}/api/store/get-ticket-count`
      )
      .then((response) => {
  ...
  const updateTicketStatus = (ticketId, status) => {
    axios
      .put(
        ...
        }/api/store/update-ticket-status/${ticketId}`,
        {
          status: status,
        }
      )
  ...
  const sendReply = (ticketId) => {
    ...
    axios
      .post(
        `${
          import.meta.env.VITE_APP_API_ENDPOINT
        }/api/store/${ticketId}/reply-to-ticket`,
        {
    ...
    <div className="flex flex-col h-screen w-full">
      ...
      <div
        className={`flex-grow flex ${
          selectedTicket ? "lg:flex-row" : "flex-col"
        }`}
      >
        <div
          className={`${
            selectedTicket ? "lg:w-3/5" : "w-full"
          } p-4 sm:p-6 bg-gray-50 rounded-xl`}
        >
